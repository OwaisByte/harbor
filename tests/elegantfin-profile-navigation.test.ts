...
} from "../src/lib/social/open-profile.ts";
...
test("tryViewMyProfile requests the normalized current handle once", () => {
  ...
  try {
    assert.equal(
      buildTryViewMyProfile(() => ({ handle: "  Alice  " }))(),
      true,
    );
...
test("Theme Studio documents the immediate boolean profile API", () => {
  assert.deepEqual(
    WINDOW_HARBOR.find(
      (entry) => entry.call === "window.harbor.tryViewMyProfile()",
    ),
    {
...
function mountElegantFin(api?: ImmediateApi) {
  ...
  new Function("window", "document", elegantFinScript())(
    windowStub,
    documentStub,
  );
  ...
  return {
    ...
    setDrawerOpen: (open: boolean) =>
      open
        ? classList.add("ef-drawer-open")
        : classList.remove("ef-drawer-open"),
